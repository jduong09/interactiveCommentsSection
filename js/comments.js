...
function createCommentStructure(commentInfo, currentUser) {
  ...
  // plus, minus icon event listeners
  plusIcon.addEventListener('click', (e) => {
    e.preventDefault();

    const plusParent = plusIcon.parentElement;

    plusParent.children[1].innerText = parseInt(plusParent.children[1].innerText) + 1;
  });

  minusIcon.addEventListener('click', (e) => {
    e.preventDefault();

    const minusParent = minusIcon.parentElement;

    minusParent.children[1].innerText = parseInt(minusParent.children[1].innerText) - 1;
  });

  const likes = document.createElement('div');
  likes.innerText = parseInt(commentInfo.score);
